...
const BeautyPage: React.FC = () => {
  ...
  const subcategories = [
    { value: 'all', label: 'すべて' },
    { value: 'cosmetics', label: 'コスメ・メイク' },
    { value: 'skincare', label: 'スキンケア' },
    { value: 'yoga', label: 'ヨガ・ストレッチ' },
    { value: 'nail-hair', label: 'ネイル・ヘアケア' },
    { value: 'mindcare', label: 'マインドケア・セルフケア' },
  ];
  ...
  const fetchPosts = async () => {
    try {
      ...
      console.log('💄 Fetching beauty posts from:', `${API_URL}/api/posts/?category=beauty`);
      const response = await fetch(`${API_URL}/api/posts/?category=beauty`, {
        headers,
      });
            ...
            {filteredPosts.map((post) => (
              ...
              >
                  ...
                  <div className="h-48 overflow-hidden relative">
                    <img 
                      ...
                      onError={(e) => {
                        ...
                        img.onerror = null;
                        img.src = `https://picsum.photos/seed/beauty-${post.id}/400/300`;
                      }}
                    />
                  </div>
                ) : post.youtube_url ? (
                ...
                <CardContent>
                  ...
                  {/* 講座参加リンク（仮想的な実装） */}
                  {post.subcategory === 'yoga' && (
                    <div className="mb-4">
                      <Button
                        variant="outline"
                        size="sm"
                        className="flex items-center space-x-1 text-carat-black border-carat-gray3 hover:bg-carat-gray1"
                      >
